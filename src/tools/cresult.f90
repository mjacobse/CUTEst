!-*-*-*-*-*-*-  C U T E S T    C R E S U L T    S U B R O U T I N E  -*-*-*-*-*-

      SUBROUTINE CUTEST_cresult( status, output, n, m,                         &
                                 X, X_l, X_u, Y, C_l, C_u )
      USE CUTEST
      INTEGER, PARAMETER :: wp = KIND( 1.0D+0 )

!  dummy arguments

      INTEGER, INTENT( OUT ) :: status
      INTEGER, INTENT( IN )  :: output, n, m
      REAL ( KIND = wp ), INTENT( IN ), DIMENSION( n ) :: X, X_l, X_u
      REAL ( KIND = wp ), INTENT( IN ), DIMENSION( m ) :: Y, C_l, C_u

!  local variables

      REAL ( KIND = wp ) :: time_now
      REAL ( KIND = wp ) :: f, l, box_complementarity, box_dual_feasibility,   &
                            general_complementarity, general_dual_feasibility
      REAL ( KIND = wp ), DIMENSION( m ) :: C
      REAL ( KIND = wp ), DIMENSION( n ) :: Dl, Z
      REAL ( KIND = wp ) :: nc2of, nc2og, nc2oh, nc2cf, nc2cg, nc2ch, nhvpr
      CHARACTER ( LEN=21 ), PARAMETER :: fmt = "(A, SS, ES25.17E3, A)"

      CALL CPU_TIME( time_now )
      WRITE( output, "(A)" ) "{"
      WRITE( output, fmt ) '"time":', time_now - CUTEST_data_global%sttime, ","

      nc2of = 0.0_wp
      nc2og = 0.0_wp
      nc2oh = 0.0_wp
      nhvpr = 0.0_wp
      nc2cf = 0.0_wp
      nc2cg = 0.0_wp
      nc2ch = 0.0_wp
      DO i = 1, SIZE( CUTEST_work_global )
        nc2of = nc2of + CUTEST_work_global( i )%nc2of
        nc2og = nc2og + CUTEST_work_global( i )%nc2og
        nc2oh = nc2oh + CUTEST_work_global( i )%nc2oh
        nhvpr = nhvpr + CUTEST_work_global( i )%nhvpr
        divisor = CUTEST_work_global( i )%pnc
        IF ( divisor <= 0.0_wp ) THEN
          divisor = 1.0_wp
        END IF
        nc2cf = nc2cf + CUTEST_work_global( i )%nc2cf / divisor
        nc2cg = nc2cg + CUTEST_work_global( i )%nc2cg / divisor
        nc2ch = nc2ch + CUTEST_work_global( i )%nc2ch / divisor
      END DO
      WRITE( output, fmt ) '"obj_function_evals":', nc2of, ","
      WRITE( output, fmt ) '"obj_gradient_evals":', nc2og, ","
      WRITE( output, fmt ) '"obj_hessian_evals":', nc2oh, ","
      WRITE( output, fmt ) '"hessian_vector_products":', nhvpr, ","
      WRITE( output, fmt ) '"constr_function_evals":', nc2cf, ","
      WRITE( output, fmt ) '"constr_gradient_evals":', nc2cg, ","
      WRITE( output, fmt ) '"constr_hessian_evals":', nc2ch, ","

      CALL CUTEST_cfn( status, n, m, X, f, C )
      WRITE( output, fmt ) '"objective_value":', f, ","
      WRITE( output, fmt ) '"primal_feasibility":', MAX( 0.0_wp,               &
        MAXVAL( X_l - X ), MAXVAL( X - X_u ),                                  &
        MAXVAL( C_l - C ), MAXVAL( C - C_u ) ), ","

      CALL CUTEST_clfg( status, n, m, X, Y, l, Dl, .TRUE. )
      Z = CUTEST_box_multipliers( X, X_l, X_u, Dl )
      Dl = Dl + Z
      WRITE( output, fmt ) '"stationarity":', MAXVAL( ABS( Dl ) ), ","

      box_dual_feasibility = 0.0_wp
      box_complementarity = 0.0_wp
      DO i = 1, n
        IF ( X_u( i ) == X_l( i ) ) THEN
          CYCLE
        ELSE IF ( ABS( X_u( i ) - X( i ) ) < ABS( X( i ) - X_l( i ) ) ) THEN
          box_dual_feasibility = MAX( box_dual_feasibility, -Z( i ) )
          box_complementarity = MAX( box_complementarity,                      &
                                     ( X( i ) - X_u( i ) ) * Z( i ) )
        ElSE
          box_dual_feasibility = MAX( box_dual_feasibility, Z( i ) )
          box_complementarity = MAX( box_complementarity,                      &
                                     ( X_l( i ) - X( i ) ) * Z( i ) )
        END IF
      END DO
      WRITE( output, fmt ) '"box_dual_feasibility":', box_dual_feasibility, ","
      WRITE( output, fmt ) '"box_complementarity":', box_complementarity, ","

      general_dual_feasibility = 0.0_wp
      general_complementarity = 0.0_wp
      DO i = 1, n
        IF ( C_u( i ) == C_l( i ) ) THEN
          CYCLE
        ELSE IF ( ABS( C_u( i ) - C( i ) ) < ABS( C( i ) - C_l( i ) ) ) THEN
          general_dual_feasibility = MAX( general_dual_feasibility, -Y( i ) )
          general_complementarity = MAX( general_complementarity,              &
                                         ( C( i ) - C_u( i ) ) * Y( i ))
        ElSE
          general_dual_feasibility = MAX( general_dual_feasibility, Y( i ) )
          general_complementarity = MAX( general_complementarity,              &
                                         ( C_l( i ) - C( i ) ) * Y( i ) )
        END IF
      END DO
      WRITE( output, fmt ) '"general_dual_feasibility":',                      &
                           general_dual_feasibility, ","
      WRITE( output, fmt ) '"general_complementarity":',                       &
                           general_complementarity, ","
      IF ( n > 1 ) THEN
        WRITE( output, fmt ) '"X":[', X( 1 ), ","
        DO i = 2, (n - 1)
          WRITE( output, "(SS, ES25.17E3, A)" ) X( i ), ","
        END DO
        WRITE( output, "(SS, ES25.17E3, A)" ) X( n ), "]"
      ELSE IF ( n > 0 ) THEN
        WRITE( output, fmt ) '"X":[', X( 1 ), "]"
      END IF

      WRITE( output, "(A)" ) "}"
      status = 0
      RETURN

!  End of subroutine CUTEST_cresult

      END SUBROUTINE CUTEST_cresult
